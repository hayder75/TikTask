...
const { verifyTransaction , initiatePayment} = require('../../utils/chapa'); 
...
const deposit = async (req, res) => {
  ...
  try {
    ...
    const appUrl = process.env.APP_URL || 'https://827c-102-213-68-161.ngrok-free.app'; // Update to your ngrok URL
    const email = user.email && user.email.includes('@') ? user.email : 'test@example.com';
    if (!email.includes('@')) {
      return res.status(400).json({ message: 'Invalid or missing user email' });
    }
    ...
    const tx_ref = `DEP-${userId}-${Date.now()}`; // Generate tx_ref once
    const chapaData = {
      amount: parseFloat(amount),
      tx_ref,
      email,
      first_name: user.name.split(' ')[0] || 'Seller',
      last_name: user.name.split(' ')[1] || '',
      callback_url: `${appUrl}/api/chapa-callback`, // Static callback URL
      return_url: `${appUrl}/dashboard?tx_ref=${encodeURIComponent(tx_ref)}`,
      customization: { title: 'Deposit' },
    };
    ...
    await transaction.save();
    console.log(`Created pending transaction: ${tx_ref}`);

    const response = await initiatePayment(chapaData);
    if (response.status === 'success') {
      console.log(`Chapa payment initiated successfully: ${response.data.checkout_url}`);
      return res.json({
    ...
    return res.status(400).json({ message: 'Chapa payment initiation failed', details: response.message });
  } catch (error) {
    console.error('Chapa deposit error:', error.message);
    const tx_ref = `DEP-${userId}-${Date.now()}`; // This might mismatch; avoid regenerating
...
const chapaCallback = async (req, res) => {
  ...
  if (!tx_ref) {
    console.log('Missing tx_ref in callback');
  ...
  if (!secretHash) {
    console.log('Secret hash not configured in .env');
    return res.status(500).json({ message: 'Server configuration error' });
  }

  const chapaSignature = req.headers['x-chapa-signature'];
  if (!chapaSignature) {
    console.log('No X-Chapa-Signature header found');
    return res.status(403).json({ message: 'Invalid request: Missing signature' });
  }
  ...
  if (computedSignature !== chapaSignature) {
    console.log(`Invalid signature. Computed: ${computedSignature}, Received: ${chapaSignature}`);
    return res.status(403).json({ message: 'Invalid signature' });
  }

  console.log(`Signature validated for tx_ref ${tx_ref}`);
  ...
  try {
    ...
    const userId = transaction.userId; // Use userId stored in transaction
    if (!userId) {
      console.log(`User ID not found in transaction for tx_ref ${tx_ref}`);
    ...
    console.log(`Verifying transaction with Chapa for tx_ref ${tx_ref}`);
    const response = await verifyTransaction(tx_ref);
    console.log(`Chapa verification response: ${JSON.stringify(response)}`);
    ...
    if (response.data && response.data.status === 'success') {
      ...
      const amount = parseFloat(response.data.amount) - (response.data.charge || 0);
      user.balance = (user.balance || 0) + amount;
      const savedUser = await user.save();
      console.log(`Balance updated, new balance: ${savedUser.balance}`);
    ...
    console.log(`Payment verification failed: ${response.data ? response.data.status : 'No data'}`);
    await Transaction.findOneAndUpdate({ tx_ref }, { status: 'failed', updatedAt: Date.now() });
    return res.status(400).json({ message: 'Payment verification failed' });
  } catch (error) {
...
const getTransactionStatus = async (req, res) => {
  ...
  try {
    ...
    if (transaction.status === 'pending') {
      console.log(`Transaction ${tx_ref} is pending, verifying with Chapa...`);
      const response = await verifyTransaction(tx_ref);
      if (response.data && response.data.status === 'success') {
        const amount = parseFloat(response.data.amount) - (response.data.charge || 0);
        user.balance = (user.balance || 0) + amount;
        ...
        await transaction.save();
        console.log(`Transaction ${tx_ref} updated to success via verification`);
      } else {
        transaction.status = 'failed';
        transaction.updatedAt = Date.now();
        await transaction.save();
        console.log(`Transaction ${tx_ref} updated to failed via verification`);
      }
    }

    return res.json({ status: transaction.status, balance: user.balance || 0 });
  } catch (error) {
...
module.exports = { getBalance, deposit, chapaCallback,getTransactionStatus , withdraw, applyPayouts , linkTelegram};